...
import { subscribeToLogs, addProgressLog, deleteProgressLog, updatePlant } from '../db';
...
export default function PlantDetail({ plant, currentUserUid, isWriterAuthorized, onBack }: PlantDetailProps) {
  ...
  const handleDeleteLog = async (logId: string) => {
    if (window.confirm("Sei sicuro di voler eliminare questo aggiornamento dal diario?")) {
      try {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6" id="plant-details-screen">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        {/* Share Button (Real-Time Sharing!) */}
        <button
          onClick={handleShare}
          id="share-link-btn"
          className="flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 text-xs font-black px-4.5 py-2.5 rounded-xl transition shadow-lg shadow-emerald-500/10 border-t border-white/20"
        >
          {copied ? (
            <>
              <Check className="w-4 h-4 animate-bounce" />
              <span>Copiato in Clipboard! 🌿</span>
            </>
          ) : (
            <>
              <Share2 className="w-4 h-4" />
              <span>Condividi Link Botanico</span>
            </>
          )}
        </button>
      </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-slate-900 border border-white/[0.08] rounded-2xl p-6 overflow-hidden relative shadow-xl">
            <div className="aspect-video w-full rounded-xl overflow-hidden bg-slate-950 mb-5 relative border border-white/[0.05] group/profile">
              <img src={plant.imageUrl} referrerPolicy="no-referrer" alt={plant.name} className="w-full h-full object-cover" />
          ...
          {/* Share instructions tutorial card */}
          <div className="bg-gradient-to-br from-indigo-950/40 to-emerald-950/20 border border-emerald-500/10 rounded-2xl p-5 shadow-lg">
            <h4 className="text-sm font-bold text-white mb-2 flex items-center gap-1.5">
              <span>👭 Condivisione con l'Amica</span>
            </h4>
            <p className="text-xs text-slate-300 leading-relaxed space-y-2">
              Invia il link di questa pagina alla tua amica! Lei potrà visualizzare tutta la cronologia fotografica e i progressi di <b>{plant.name}</b> in tempo reale, proprio come li carichi, senza bisogno di account.
            </p>
            <div className="mt-3.5 flex bg-slate-950/60 p-2 rounded-lg border border-white/[0.08] items-center justify-between text-[11px] font-mono text-slate-400">
              <span className="truncate max-w-[200px]">{window.location.origin + "?plantId=" + plant.id}</span>
              <button
                onClick={handleShare}
                className="text-emerald-400 hover:text-emerald-300 flex items-center gap-1 flex-shrink-0"
              >
            ...
            {loading ? (
              ...
              <div className="relative pl-6 lg:pl-10 space-y-8">
                ...
                {logs.map((log, index) => {
                  ...
                  const formattedLogDate = new Date(log.date).toLocaleDateString('it-IT', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric'
                  });
                      ...
                      <div className="bg-white/[0.02] border border-white/[0.05] hover:border-emerald-400/20 rounded-2xl p-5 hover:bg-white/[0.03] transition duration-200">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2.5 mb-3.5">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-white font-mono">{formattedLogDate}</span>
                          ...
                          {isOwner && (
                            <button
                              ...
                              className="opacity-0 group-hover/item:opacity-100 p-1.5 text-slate-500 hover:text-rose-400 transition rounded hover:bg-rose-500/10 cursor-pointer"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-12 gap-5">
                          ...
                          <div className="sm:col-span-5 aspect-square rounded-xl overflow-hidden bg-slate-950 border border-white/[0.08] relative group/photo">
                            <img src={log.imageUrl} referrerPolicy="no-referrer" alt="Progress Photo" className="w-full h-full object-cover group-hover/photo:scale-105 transition duration-300" />
                          </div>
